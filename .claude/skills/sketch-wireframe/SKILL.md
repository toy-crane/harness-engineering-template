...
각 화면에서 빈 상태, 로딩 상태, 에러 상태를 의식적으로 고려한다.

- 적용되는 상태는 맥락에 맞는 위치에 시각적으로 표현한다
- 적용되지 않는 상태는 Screen Notes에 "N/A" 사유를 명시한다
...
확정된 레이아웃 위에 나머지 시나리오 중 고유한 UI 상태를 가진 것만 탭으로 추가한다. 동작 검증 시나리오는 탭 없이 관련 화면의 Screen Notes에 기록한다.
