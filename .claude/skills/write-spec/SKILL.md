...
- 모든 질문은 `AskUserQuestion` 도구로 한다. 선택지를 텍스트로 나열하지 않는다
  - header: 질문 주제를 12자 이내로 (예: "에러 처리", "인증 방식")
  - options: 2-4개 선택지. 각 label은 1-5단어, description에 트레이드오프를 설명한다
  - 추천 선택지가 있으면 label 끝에 "(Recommended)"를 붙이고 첫 번째에 배치한다
  - UI 레이아웃처럼 시각적 비교가 필요하면 preview 필드를 활용한다
- 한 번에 질문 하나. 응답을 받기 전까지 다음 질문으로 진행하지 않는다
...
갭이 있으면 `AskUserQuestion`(multiSelect: true)으로 반영할 갭을 선택받고, Step 4의 규칙을 적용하여 추가한다.
