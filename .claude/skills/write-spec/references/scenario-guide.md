...
각 성공 기준 항목은 구체적인 쌍이다: **입력 → 관찰 가능한 출력**. 한 항목은 하나(혹은 소수의) 테스트 단언에 깔끔하게 매핑된다.
...
## 3. 시나리오 대신 불변 규칙을 쓸 때

어떤 규칙은 *모든* 경로에 적용되므로 단일 Given/When/Then으로 표현할 수 없다. 세 범주는 시나리오가 아니라 불변 규칙(Invariants) 섹션에 속한다:

| 범주 | 예시 |
|---|---|
| **보안 / 프라이버시** | "다른 사용자의 비공개 드래프트는 어떤 경로(UI, URL, 검색, RSS)로도 보이지 않는다" |
| **성능** | "모든 페이지가 4G 연결에서 2초 안에 first contentful paint에 도달한다" |
| **데이터 일관성** | "표시되는 업보트 수는 기록된 투표 수와 항상 일치한다" |

세 개 이상의 시나리오에 거의 동일한 Then을 쓰고 있다면, 그 규칙은 아마 불변 규칙에 속한다.

이런 교차 규칙이 없는 feature라면 불변 규칙 섹션을 통째로 생략한다.
