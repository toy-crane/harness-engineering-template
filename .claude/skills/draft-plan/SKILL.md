...
- 계획 시점과 실행 시점의 코드베이스가 다를 수 있다. 따라서 task에는 구현 대상(What)과 수용 기준만 포함하고, 절차(How)는 포함하지 않는다
- task의 참조에는 executor가 스스로 찾을 수 없는 외부 소스만 포함한다. 이미 컨텍스트에 로드되는 스킬/CLAUDE.md 규칙은 적지 않는다. 스킬 참조가 필요하면 스킬명 + 키워드 수준으로 영역만 힌트한다
