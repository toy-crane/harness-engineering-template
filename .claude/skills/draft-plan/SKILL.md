...
Input: `$ARGUMENTS`로 주어진 feature 이름. 전제는 `artifacts/<feature>/spec.md`.
Output: `artifacts/<feature>/plan.md` — Task 단위로 쪼갠 TDD 기반 구현 계획.

spec이 "무엇을 만들지"라면, plan은 "어떤 순서로, 어떤 단위로, 어떻게 검증하며 만들지"다. 이 경계를 지키는 이유: 구현 전략이 바뀌어도 spec은 흔들리지 않아야 하고, 반대로 plan은 구현 중 유일한 진행 지도가 되어야 하기 때문이다.

## Step 1: 전제 조건 & Plan Mode
...
읽기 전용 모드로 작동한다. 이 스킬이 만드는 유일한 파일은 `artifacts/<feature>/plan.md` 뿐이며, 그 외 프로젝트 파일은 생성·수정·삭제하지 않는다.
...
Step 2에서 확정한 각 스킬의 SKILL.md를 읽는다. plan이 실행 중 로드되는 규칙과 어긋나면 execute 단계에서 충돌이 터지기 때문이다.

`references/plan-template.md`를 읽고 그 형식을 따른다. 원칙은 아래, 형식은 template가 담당한다.

### Vertical Slicing

각 Task는 end-to-end로 동작하고 테스트 가능한 slice여야 한다. horizontal layer(DB-only, API-only, UI-only)로 쪼개지 않는다.

**왜**: horizontal로 쪼개면 각 Task만으로는 end-to-end 테스트가 돌지 않아 회귀가 통합 시점에 한꺼번에 터진다. vertical이면 Task 하나가 끝났을 때 기능이 실제로 살아 있다.
...
**왜**: L 이상이 되면 리뷰가 사실상 불가능해지고 되돌리기 비용이 급격히 커진다.
...
각 Task의 **수용 기준**은 **담당 시나리오**의 성공 기준에서 파생된 자연어 결과 체크리스트다. spec의 구체적 값을 사용하되, 결과는 외부에서 관찰 가능해야 한다.

**왜**: 수용 기준이 내부 상태나 함수 호출을 가리키면 그 테스트는 구현에 결합되어 리팩터링마다 깨진다.
...
구체적 명시 형태는 `references/plan-template.md`를 본다.
...
Task 2-3개마다 체크포인트를 삽입한다. 체크포인트는 모든 테스트 통과 + 빌드 성공 + vertical slice의 end-to-end 동작을 검증한다.

**왜**: 중간 점검이 없으면 누적 실패가 끝에서 한꺼번에 드러나 원인을 거꾸로 추적해야 한다. 2-3개마다 끊으면 실패 구간이 좁아진다.
...
승인되면 `/execute-plan <feature>`로 안내한다.
