...
위 입력를 읽고, 구현에 필요하지만 아직 결정되지 않은 사항을 찾는다.
...
- 테스트 뼈대 변환(Red)을 가장 먼저 배치한다. 선행 작업이 필요하면 사유와 함께 앞에 둔다
- 이후 task에서 구현하며 Green으로 전환한다
...
`plan-reviewer` 에이전트를 호출하여 위 입력와 plan.md 사이의 불일치를 검증한다.
