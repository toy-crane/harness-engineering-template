...
### spec 테스트 작성 규칙
- spec 테스트(`*.spec.test.tsx`)는 생성 task 이후 수정 금지. 테스트가 실패하면 구현을 수정한다 (spec.yaml에서 파생된 수용 기준이 계약이므로, 테스트 변경은 계약 위반이다)
...
의존성이 적은 것부터 구현한다. 역방향 의존은 금지한다. (순환 의존 방지 및 빌드 순서 안정화를 위해)
